...
//
// Prototype 31 optimizes the Prototype 25 qualified nonlinear renderer and adds
// chroma-aware neutral-correction diagnostics. Kodak's published C/M/Y curves
// remain the spectral basis. The density-1 reference amplitudes remain the
// Prototype 18 D55/CIE-1931 colorimetric calibration.
//
// The former Prototype 19 linear growth law is retained explicitly for A/B
// diagnostics. Active synthesis maps each normalized Status-A record increment
// through a 161-point monotone-cubic calibration derived from the P21/P23/P24
// neutral-preserving inverse. The mapping is calibration-derived; it is not
// claimed to be measured multi-density Kodak dye physics.
class PrintDyeModel
{
public:
    struct Diagnostics
    ...
    // Prototype 25 nonlinear-growth synthesis retained explicitly for A/B.
    // The canonical one-argument synthesis remains the P25 mapping because
    // the P30 qualified law additionally requires the matched neutral-manifold
    // density at the same P19 viewed tone.
    ...
    // Prototype 30 qualified production synthesis.
    // Thresholds are fixed by the P29 DigitalSG qualification:
    //   relative RMS <= 0.012 -> exact P25
    //   relative RMS >= 0.018 -> exact P19
    //   smooth transition only between those bounds.
    SampledCurve synthesize_density_p30(
        const FilmDensity& density,
        const FilmDensity& neutral_density) const;
    SampledCurve synthesize_transmittance_p30(
        const FilmDensity& density,
        const FilmDensity& neutral_density) const;


    // Prototype 26 diagnostic retained for A/B comparison.
    SampledCurve synthesize_density_p26(
        const FilmDensity& density,
        float fade_end) const;
    SampledCurve synthesize_transmittance_p26(
        const FilmDensity& density,
        float fade_end) const;
    ...
    // Prototype 27 diagnostic: distance is measured relative to the
    ...
    float neutrality_distance_relative(
        const FilmDensity& density,
        const FilmDensity& neutral_density) const;
    // Prototype 28 metric-selection audit candidates. These are diagnostic
    // record-space distances relative to the matched neutral manifold.
    ...
    float neutrality_distance_relative_log_rms(
        const FilmDensity& density,
        const FilmDensity& neutral_density) const;
    // Prototype 29 diagnostic: relative-RMS metric with an exact P25 neutral
    // plateau and exact P19 chromatic release.
    FilmDensity blended_reference_amplitudes_p29(
        const FilmDensity& density,
        const FilmDensity& neutral_density,
        float neutral_end,
        float chroma_start) const;
    SampledCurve synthesize_density_p29(
        const FilmDensity& density,
        const FilmDensity& neutral_density,
        float neutral_end,
        float chroma_start) const;
    SampledCurve synthesize_transmittance_p29(
        const FilmDensity& density,
        const FilmDensity& neutral_density,
        float neutral_end,
        float chroma_start) const;
    FilmDensity blended_reference_amplitudes_p27(
        const FilmDensity& density,
        const FilmDensity& neutral_density,
        float fade_end) const;
    SampledCurve synthesize_density_p27(
        const FilmDensity& density,
        const FilmDensity& neutral_density,
        float fade_end) const;
    SampledCurve synthesize_transmittance_p27(
        const FilmDensity& density,
        const FilmDensity& neutral_density,
        float fade_end) const;
    ...
    //
    // Uses the same Dmin/reference-record coordinate as P19:
    //   n = max(0, (D - Dmin) / (Dref - Dmin))
    //
    // but omits the Prototype-18 unequal C:M:Y metameric amplitudes.
    ...
    // Prototype 19 residual-free but linearly scaled reference model.
    SampledCurve synthesize_density_p19(const FilmDensity& density) const;
    SampledCurve synthesize_transmittance_p19(const FilmDensity& density) const;

    // Prototype 18-and-earlier residual-based synthesis retained only for
